...
def merge_patch_subset_outputs(patch_info, patch_outputs, input_pcd, device, dtype):
    merged_points = []
    merged_weights = []
    gt_subset_points = []
    gt_subset_labels = []
    labels_available = True
    num_input_points = int(patch_info["num_input_points"])
    owned_filled = torch.zeros(num_input_points, device=device, dtype=torch.bool)
    ...
    for patch_entry in patch_outputs:
        ...
        owned_global_idx = patch_entry.get("owned_global_idx")
        owned_out_label = patch_entry.get("owned_out_label")
        if owned_global_idx is None:
            patch_input_idx = patch_info["patch_input_idx"][patch_id]
            owned_input_mask = patch_info["owned_input_mask"][patch_id]
            owned_local_idx = torch.nonzero(owned_input_mask, as_tuple=False).flatten()
            if owned_local_idx.numel() > 0:
                owned_global_idx = patch_input_idx.index_select(0, owned_local_idx)
                if owned_out_label is None:
                    out_label = patch_entry["patch_meta"].get("out_label")
                    if out_label is not None:
                        owned_out_label = out_label.index_select(0, owned_local_idx)

        if owned_global_idx is None or owned_global_idx.numel() <= 0:
            continue

        owned_global_idx = owned_global_idx.to(device=device, dtype=torch.long)
        if torch.any(owned_filled.index_select(0, owned_global_idx)):
            raise RuntimeError("Subset patch merge encountered duplicate owned input indices.")
        owned_filled[owned_global_idx] = True
        gt_subset_points.append(input_pcd[:, :, owned_global_idx].squeeze(0))

        if owned_out_label is None:
            labels_available = False
        else:
            gt_subset_labels.append(owned_out_label.to(device=device, dtype=dtype).reshape(-1))

    if not merged_points:
        raise RuntimeError("No patch outputs were selected during subset merge.")
    if not gt_subset_points:
        raise RuntimeError("Subset merge did not collect any owned ground-truth points.")

    merged_pts = torch.cat(merged_points, dim=1).unsqueeze(0).contiguous()
    gt_subset_pts = torch.cat(gt_subset_points, dim=1).unsqueeze(0).contiguous()

    merged_final_w = None
    if merged_weights:
        merged_final_w = torch.cat(merged_weights, dim=1).unsqueeze(0).contiguous()

    out_label_subset = None
    if labels_available:
        if gt_subset_labels:
            out_label_subset = torch.cat(gt_subset_labels, dim=0).view(1, 1, -1).contiguous()
        else:
            out_label_subset = torch.zeros((1, 1, gt_subset_pts.shape[-1]), device=device, dtype=dtype)

    return merged_pts, gt_subset_pts, merged_final_w, out_label_subset
