...
def surrogate_registry_path(args):
    ...
    data_name = _safe_component(getattr(args, "surrogate_data", ""), f"{getattr(args, 'dataname', 'data')}_{getattr(args, 'dataset_name', 'set')}") # surrogate_data名を決める
    time_name = _safe_component(getattr(args, "surrogate_time", ""), getattr(args, "time", "run")) # surrogate_time名を決める
    return os.path.join(root, method_name, f"{data_name}_{time_name}.pth") # 指定形式の保存先パスを返す


def latest_surrogate_registry_path(args):
    exact_path = surrogate_registry_path(args) # 指定形式の現在run保存先を取得する
    if exact_path is None:
        return None # 保存機能が無効なら候補なしにする
    data_name = _safe_component(getattr(args, "surrogate_data", ""), f"{getattr(args, 'dataname', 'data')}_{getattr(args, 'dataset_name', 'set')}") # 最新検索に使うデータ名を決める
    candidates = glob.glob(os.path.join(os.path.dirname(exact_path), f"{data_name}_*.pth")) # 同じmethod/dataの保存済みSurrogateを列挙する
...
def load_surrogate_registry_state(args, loss, writer=None):
    if not bool(getattr(args, "surrogate_pretrain_resume", True)): # resume無効時は共有Surrogateを読まない
        return False, surrogate_registry_path(args) # 呼び出し側に候補パスだけ返す
    if bool(getattr(args, "surrogate_pretrain_force_retrain", False)): # 強制再学習時は共有Surrogateを読まない
        return False, surrogate_registry_path(args) # 呼び出し側に候補パスだけ返す
    path = surrogate_registry_path(args) # 指定形式の共有Surrogateパスを取得する
    if path is None or not os.path.exists(path):
        if bool(getattr(args, "surrogate_registry_load_latest_if_missing", True)):
