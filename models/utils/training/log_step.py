    ...
    *,
    ...
    if str(comp_debug.get("loss_mode", "")).strip().lower() == "compression_primary":
        writer.write(
            ...
            f"geom={_fmt(comp_debug.get('cp_geom_block_scaled', float('nan')))}"
            f"(raw={_fmt(comp_debug.get('cp_geom_block_raw', float('nan')))}, "
            f"tau={_fmt(comp_debug.get('cp_geom_tau', float('nan')))}, "
            f"lambda={_fmt(comp_debug.get('cp_geom_lambda', float('nan')))}, "
            f"dL/dGeom={_fmt(comp_debug.get('cp_geom_scalar_grad_scale', float('nan')))}), "
    ...
    full_cloud_amount_actual_requested_count = _prefer_number("full_cloud_amount_actual_requested_count", 0.0)
    ...
    writer.write(
        ...
        f"FCAActualRequested={_fmt_int(full_cloud_amount_actual_requested_count)}, "
