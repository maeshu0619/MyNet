...
def log_point_edit_stats(writer, train_edit_stats, step, num_steps):
    ...
    writer.write(
        ...
        f"mode={'voxel' if bool(train_edit_stats.get('voxel_operation_mode', False)) else 'point'}, "
        ...
        f"voxel_move_ratio={float(train_edit_stats.get('voxel_move_ratio_percent', 0.0)):.4f}%, "
        f"voxel_add_ratio={float(train_edit_stats.get('voxel_add_ratio_percent', 0.0)):.4f}%, "
        f"voxel_drop_ratio={float(train_edit_stats.get('voxel_drop_ratio_percent', 0.0)):.4f}%, "
