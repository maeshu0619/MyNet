...
try:
    ...
    def normalize(pointcloud):
        centroid = np.mean(pointcloud, axis=0, keepdims=True)
        pointcloud = pointcloud - centroid
        scale = np.max(np.sqrt(np.sum(pointcloud ** 2, axis=1, keepdims=True)))
        return (pointcloud / max(float(scale), 1e-12)).astype("float32")
