            ...
            for step, pts in enumerate(loader): # Step開始
                ...
                if den6_online_full_cloud:
                    ...
                    writer.write(
                        ...
                        f"selection_mode={str(audit_plan.get('selection_mode', ''))}, "
                        f"factorized_group={str(audit_plan.get('factorized_exploration_group', ''))}, "
                        f"factorized_counts={dict(audit_plan.get('factorized_exploration_counts') or {})}, "
                        ...
                        f"plan_critic=(value={float(audit_compression.get('den6_online_policy_plan_critic_value', 0.0) or 0.0):.6f}, "
                        f"uncertainty={float(audit_compression.get('den6_online_policy_plan_critic_uncertainty', 0.0) or 0.0):.6f}, "
                        f"loss={float(audit_compression.get('den6_online_policy_plan_critic_loss_weighted', 0.0) or 0.0):.6g}, "
                        f"replay={int(audit_compression.get('den6_online_policy_plan_critic_replay_count', 0) or 0)}, "
                        f"pearson={float(audit_compression.get('den6_online_policy_plan_critic_replay_pearson', 0.0) or 0.0):.4f}, "
                        f"spearman={float(audit_compression.get('den6_online_policy_plan_critic_replay_spearman', 0.0) or 0.0):.4f}, "
                        f"policy_confidence={float(audit_compression.get('den6_online_policy_plan_critic_policy_confidence', 0.0) or 0.0):.4f}), "
                        f"actual_set_credit=(raw={float(audit_compression.get('den6_online_policy_actual_set_contrast_raw', 0.0) or 0.0):.6g}, "
                        f"weighted={float(audit_compression.get('den6_online_policy_actual_set_contrast_weighted', 0.0) or 0.0):.6g}, "
                        f"ops={int(audit_compression.get('den6_online_policy_actual_set_contrast_operations', 0) or 0)}, "
                        f"pairs={int(audit_compression.get('den6_online_policy_actual_set_contrast_pairs', 0) or 0)}, "
                        f"evidence={float(audit_compression.get('den6_online_policy_actual_set_contrast_evidence', 0.0) or 0.0):.4f}, "
                        f"incumbent_update={bool(audit_compression.get('den6_online_policy_actual_set_incumbent_updated', False))}), "
                        f"policy_global_available={bool(audit_compression.get('den6_online_policy_global_actual_credit_available', False))}, "
                        f"policy_global_confidence={float(audit_compression.get('den6_online_policy_global_actual_credit_confidence', 0.0) or 0.0):.4f}, "
                        f"policy_global_weight={float(audit_compression.get('den6_online_policy_global_actual_credit_weight', 0.0) or 0.0):.4f}, "
                        ...
                        f"outside_heuristic_budget={dict(audit_plan.get('selected_outside_heuristic_budget_rate') or {})}, "
                        f"outside_exploration_support={dict(audit_plan.get('selected_outside_exploration_support_rate') or {})}, "
                        f"entropy={dict(audit_plan.get('candidate_entropy_by_operation') or {})}, "
                        ...
                        f"exploration_top1=(by_op={dict(audit_plan.get('exploration_top1_changed') or {})}, "
                        f"rate={float(audit_plan.get('exploration_top1_changed_rate', 0.0) or 0.0):.3f}, "
                        f"heuristic_selected={dict(audit_plan.get('exploration_heuristic_top1_selected') or {})}, "
                        f"selected_set_change={dict(audit_plan.get('exploration_selection_change_rate') or {})}), "
                        ...
                        f"learning_adaptive={bool(audit_plan.get('learning_adaptive_exploration', False))}, "
                        f"quality_factor={float(audit_plan.get('plan_exploration_factor', 1.0) or 0.0):.3f}, "
                        f"factor_by_op={dict(audit_plan.get('operation_exploration_factors') or {})}, "
                        f"support={dict(audit_plan.get('exploration_support_sizes') or {})}, "
                        f"reserve_by_op={dict(audit_plan.get('exploration_support_reserve_factors') or {})}, "
                        ...
                        f"gate_actual_share={dict(audit_plan.get('operation_gate_selected_shares') or {})}, "
                        f"gate_share_l1={float(audit_plan.get('operation_gate_share_l1_error', 0.0) or 0.0):.6g}, "
                        ...
                        f"amount_delta=(mean={float(audit_plan.get('delta_amount_mean', 0.0) or 0.0):.7f}, "
                        f"std={float(audit_plan.get('delta_amount_std', 0.0) or 0.0):.7f}, "
                        f"abs_max={float(audit_plan.get('delta_amount_abs_max', 0.0) or 0.0):.7f}), "
                        ...
                        f"gpu_wait={float(actual_audit.get('gpu_admission_wait_time', 0.0) or 0.0):.3f}s, "
                        ...
                        f"point_transformer_working_set_bypassed={int(static_node_cache.get('point_transformer_working_set_bypassed', 0) or 0)})"
